...
export default {
  ...
  data() {
    return {
      showModal: true,
      sinNumber: "", // Add SIN number to data
      reason: "",
      estimatedCostKnown: "",
      estimatedCost: "",
      costKnownStatus: "",
  ...
  mounted() {
    // Get SIN number from session storage when component mounts
  ...
  methods: {
    getSinFromSession() {
      // Try to get SIN number from different possible session storage keys used by airfield runway form
      this.sinNumber =
        this.sinNumber || // Use prop if provided
        sessionStorage.getItem("airfieldSinNumber") || // Primary key used in airfield form
      ...
      // If no SIN found, show warning
      if (!this.sinNumber) {
        console.warn(
          "No SIN number found in session storage. Please ensure the airfield runway form was completed."
        );
        Swal.fire({
          icon: "warning",
    ...
    saveAndRedirect() {
      // Check if SIN number is available
      if (!this.sinNumber) {
        Swal.fire({
          icon: "error",
        ...
        if (result.isConfirmed) {
          try {
            ...
            Swal.fire({
              ...
              this.$router.push("/construction/usage-info");
            });
          } catch (error) {
