              ...
              <div class="text-container">
                <nav aria-label="breadcrumb">
                  <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">List of Districts</li>
                  </ol>
                </nav>
                <hr />
                <div>
                  <table style="text-align: left;" class="table">
                    ...
                    <tr v-for="item in itemsdistrict" :key="item">
                      <td> {{ item.district }}</td>
                      <router-link :to="{ name: 'DistrictManagementView', params: { id: item.id } }"
                        class="btn btn-sm btn-success" style="margin: 5px;"> View
                      </router-link>
                      <router-link :to="{ name: 'districtUpdate', params: { id: item.id, id2: item.district } }"
      ...
      <div class="a-box align-self-center text-align-left">
        <div class="card card-container">
          <Form @submit="handleSave" :validation-schema="schema">
            <Field type="hidden" name="id" v-model="this.$route.params.id" />
            <div v-if="!successful">
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <li class="breadcrumb-item active" aria-current="page"><a
              ...
              <div class="form-group row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                  <div class="form-group">
                    ...
                    <Field name="name" type="text" class="form-control" v-model="this.$route.params.id2" />
...
export default {
  ...
  methods: {
      ...
      this.$swal.fire(
        {
          ...
          if (result.isConfirmed) {
            ...
            district_ds_gn_service.updateDistrict(data).then(
              (response) => {
    ...
    changeMessageActive() {
      return axios
        .get(resources_files.backend_url_internal() + '/get/api/production/user/internal/backend/restapi/get/all/contdistrict')
        .then(response => {
          this.itemsdistrict = response.data;
