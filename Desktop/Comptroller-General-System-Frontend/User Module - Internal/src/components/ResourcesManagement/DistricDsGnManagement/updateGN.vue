            ...
            <div v-if="!successful">
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <li class="breadcrumb-item active" aria-current="page"><a
                      onclick='window.location.href = "/cgoInternal/"' class="btn-sm btn-link">Home</a></li>
                  <li class="breadcrumb-item active" aria-current="page">Update Grama Niladhari</li>
                </ol>
              </nav>
              <Field name="id" type="hidden" class="form-control" v-model="this.$route.params.id" />
              <Field name="id_pk" type="hidden" class="form-control" v-model="this.$route.params.id2" />
...
export default {
  ...
  methods: {
      ...
      this.$swal.fire(
        {
          ...
          if (result.isConfirmed) {
            ...
            district_ds_gn_service.updateGN(data).then(
              (response) => {
    ...
    changeMessageActive() {
      ...
      return axios
        .get(resources_files.backend_url_internal() + '/get/api/production/user/internal/backend/restapi/get/one/contds/' + this.dataParam2)
        .then(response => {
